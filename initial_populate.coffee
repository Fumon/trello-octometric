...
getCardsBefore = (date) ->
  ...
  filterParams.before = date if date?
...
allcards.then((val) ->
  ...
  for card in val.data
    do (card) ->
      return_list.push new Promise (resolve, reject) ->
        # Database connection
        pg.connect
          user: 'postgresuser'
          database: 'database'
          ...
          (err, client, done) ->
            ...
            q.on 'end', (end) ->
              ...
              resolve new Date(card.dateLastActivity)
  return_list
, (err) ->
  ...
  new Promise (resolve, reject) ->
    ...
    console.log "Promise returns good"
  , (err) ->
